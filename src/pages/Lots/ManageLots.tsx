...
export default function ManageLots() {
  const { currentUser, lots, addLot } = useStore();
  ...
  if (showForm) {
    return (
      <div className="max-w-3xl">
        ...
        <h1 className="text-3xl font-bold text-slate-900 mb-6">List New Lot</h1>

        <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-900 mb-1">Lot Code</label>
              <input 
                type="text" 
                required 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-slate-900 mb-1">Grade</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-medium text-slate-900 mb-1">Processing Method</label>
                <select 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-slate-900 mb-1">Region</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-medium text-slate-900 mb-1">Quantity (quintals)</label>
                <input 
                  type="number" 
                  required 
                  min={1} 
            ...
            <div>
              <label className="block text-sm font-medium text-slate-900 mb-1">Asking Price (ETB)</label>
              <input 
                type="number" 
                required 
                min={1} 
            ...
            <div>
              <label className="block text-sm font-medium text-slate-900 mb-1">Harvest Season</label>
              <input 
                type="text" 
                required 
            ...
            <div>
              <label className="block text-sm font-medium text-slate-900 mb-1">Cupping Notes</label>
              <textarea 
                rows={3}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-900 mb-2">Upload Documents</label>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-1">My Lots</h1>
          <p className="text-slate-500 text-sm">{myLots.length} my lots</p>
        </div>
        <Button onClick={() => setShowForm(true)} className="bg-navy hover:bg-navy/90 text-white gap-2 rounded-lg py-2">
          <Plus className="h-4 w-4" /> List New Lot
        </Button>
      </div>
      ...
      {myLots.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center flex flex-col items-center">
          ...
          <h3 className="text-lg font-bold text-slate-900 mb-2">No lots listed</h3>
          <p className="text-slate-500 mb-6 max-w-sm">You haven't listed any coffee lots yet. Click the button above to create your first listing.</p>
          ...
          {myLots.map(lot => (
            <div key={lot.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="p-5">
                ...
                <h3 className="text-lg font-bold text-slate-900 mb-1">Grade {lot.grade} &middot; {lot.processingMethod}</h3>
                ...
                <div className="border-t border-slate-100 pt-4 mt-2">
                  <div className="text-sm text-slate-500 mb-1">{lot.quantity} quintals</div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-xl font-bold text-slate-900">{lot.price.toLocaleString()} ETB</span>
                    <span className="text-xs text-slate-500">per quintal: {Math.round(lot.price / lot.quantity).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
