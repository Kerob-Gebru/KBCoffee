...
export default function QualityReports() {
  const { currentUser, lots, qualityReports, addQualityReport } = useStore();
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-2xl font-bold text-navy uppercase tracking-wide">Quality Certifications</h1>

      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-sm shadow-sm shadow-black/5 border border-slate-100">
          <h2 className="text-lg font-bold uppercase tracking-wide text-navy mb-4 border-b border-slate-100 pb-2">Submit Grading Report</h2>
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Select Lot for Inspection</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Moisture (%)</label>
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Defect Count</label>
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Cup Score (0-100)</label>
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Final Grade</label>
            ...
            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Certification Number</label>
            ...
            <Button type="submit" variant="default" className="w-full">Submit Certified Report</Button>
          </form>
        </div>

        <div className="bg-white p-6 rounded-sm shadow-sm shadow-black/5 border border-slate-100">
          <h2 className="text-lg font-bold uppercase tracking-wide text-navy mb-4 border-b border-slate-100 pb-2">My Reports</h2>
              ...
              return (
                <div key={report.id} className="p-4 border border-slate-100 rounded-sm bg-slate-50">
                  <div className="flex justify-between mb-2">
                    <span className="font-bold text-navy uppercase text-sm">Lot: {lot?.name || report.lotId}</span>
                    <span className="text-[10px] bg-green-100 text-green-800 px-2 py-1 rounded-sm uppercase font-bold tracking-widest">Grade {report.gradeClassification}</span>
                  </div>
                  <div className="text-xs text-slate-600 grid grid-cols-2 gap-2 mt-3 border-t border-slate-200 pt-3">
                    <div><span className="text-slate-400 uppercase font-bold text-[9px] block">Cup Score</span> {report.cupScore}</div>
                    <div><span className="text-slate-400 uppercase font-bold text-[9px] block">Moisture</span> {report.moisture}%</div>
                    <div className="col-span-2 text-[10px] text-slate-500 mt-1 uppercase font-bold"><span className="text-navy">Cert:</span> {report.certificationNumber}</div>
                  </div>
                </div>
              );
            })}
